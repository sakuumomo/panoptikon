...
- ROCm (`panoptikon/src/rocm_env.rs`): local Inferio prepends host HIP/HSA
  lib dirs to worker `LD_LIBRARY_PATH` (`ROCM_PATH`/`HIP_PATH` lib,
  `/opt/rocm/lib`, NixOS profile lib when it has HIP, `/run/opengl-driver/lib`).
  `panoptikon setup --accelerator rocm` probes a trivial HIP kernel after
  sync; failure aborts setup (wrong wheel arch / missing userspace).
